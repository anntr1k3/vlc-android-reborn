...
- **Media Library Preallocated Batch Loading**: Replaced un-sized `mutableListOf()` in `MedialibraryProvider.loadPagedList` with exact `ArrayList<MediaWrapper>(count)` pre-allocation, eliminating multiple internal array resize allocations and GC churn during playback queue preparation on large libraries (10,000+ tracks).
